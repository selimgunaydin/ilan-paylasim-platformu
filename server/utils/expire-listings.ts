...
import { sendExpirationWarningEmail, sendExpirationEmail } from "../mail";
...
export async function deactivateExpiredListings() {
  try {
    ...
    console.log('Süresi dolan ilanlar:', activeListings.map(l => ({ id: l.id, title: l.title, expiresAt: l.expiresAt })));
    ...
    result.forEach(listing => {
      console.log(`Pasif yapılan ilan: ID=${listing.id}, Başlık=${listing.title}, Bitiş Tarihi=${listing.expiresAt}`);
      // Süre dolum bildirimi gönder
      if (listing.userId) {
        sendExpirationEmail(listing);
      }
    });
...
export async function checkExpirationWarnings() {
  try {
    ...
    console.log(`${expiringListings.length} adet ilanın süresi 3 gün içinde dolacak`);
    console.log('Uyarı gönderilecek ilanlar:', expiringListings.map(l => ({ 
      id: l.id, 
      title: l.title, 
    ...
    for (const listing of expiringListings) {
      if (listing.userId) {
        try {
          await sendExpirationWarningEmail(listing);
          console.log(`Süre uyarı e-postası gönderildi: İlan ID=${listing.id}, Başlık=${listing.title}`);
        } catch (error) {
          console.error(`Uyarı e-postası gönderme hatası (İlan ID=${listing.id}):`, error);
        }
      }
    }
