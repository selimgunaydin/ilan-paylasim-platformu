...
export default function KategorilerPage() {
  ...
  const reorderMutation = useMutation({
    mutationFn: (updates: { id: number; order: number; parentId: number | null }[]) =>
  ...
  const addMutation = useMutation({
    mutationFn: async (newCat: { name: string; parentId: number | null; slug: string }) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (categoryId: number) => {
      ...
      const checkResponse = await fetch(`/api/admin/categories/${categoryId}/can-delete`);
      ...
      // Kategoriyi sil
      const deleteResponse = await fetch(`/api/admin/categories/${categoryId}/force`, {
        method: "DELETE",
      });
  ...
  if (error) return <div>Hata: {error instanceof Error ? error.message : 'Bir hata oluştu'}</div>;
    ...
    <div className="p-8">
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => addMutation.mutate(newCategory)}>
            Ekle
          </Button>
        </div>
      </div>
          ...
          {categories.map((category) => {
            ...
            return (
              <SortableCategory
                ...
                onDelete={listingCounts[category.id] > 0 ? undefined : () => setCategoryToDelete(category)}
              />
            );
          })}
        </SortableContext>
      ...
      <AlertDialog open={!!categoryToDelete} onOpenChange={(open) => !open && setCategoryToDelete(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setCategoryToDelete(null)}>İptal</AlertDialogCancel>
